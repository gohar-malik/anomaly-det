    ...
    if not os.path.isfile(save_model_path):
        ...
        for epoch in range(epochs):
            ...
            if best_acc < test_metrics['acc']:
                state = {'best_epoch':epoch, 'best_test_acc':test_metrics['acc']}
                json.dump(state, open(os.path.join(savedir, f'{exp_name}.json'),'w'), indent=4)

                torch.save(model.model.state_dict(), save_model_path)
                
                _logger.info('Best Accuracy {0:.3%} to {1:.3%}'.format(best_acc, test_metrics['acc']))

                best_acc = test_metrics['acc']

        _logger.info('Best Dev Metric: {0:.3%} | Best Test Metric: {0:.3%} (epoch {1:})'.format(state['best_test_acc'], state['best_epoch']))
    else:
        if devloader is not None:
            eval_metrics = test(model, devloader, criterion, log_interval, 'DEV', device)
        test_metrics = test(model, testloader, criterion, log_interval, 'TEST', device)

        state = {'best_test_acc':test_metrics['acc']}
...
if __name__=='__main__':
    ...
    parser.add_argument('--modelname',type=str,choices=['vgg19','resnet34', 'resnet18'])
    ...
    parser.add_argument('--dev_ratio',type=float,default=0.0,help='dev set split ratio')
    ...
    parser.add_argument('--seed',type=int,default=42,help='223 is my birthday')
